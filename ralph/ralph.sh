...
for i in $(seq 1 $MAX_ITERATIONS); do
  ...
  if [[ "$TOOL" == "amp" ]]; then
    ...
    # GitHub Copilot CLI: use -p for non-interactive mode with --allow-all and --autopilot
    RALPH_PROMPT=$(cat "$SCRIPT_DIR/CLAUDE.md")
    OUTPUT=$(copilot -p "$RALPH_PROMPT" --allow-all --autopilot 2>&1 | tee /dev/stderr) || true
  fi
